...
export default function CalendarView() {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      // Convert to ISO strings with timezone (Z for UTC)
  ...
  const createEvent = async () => {
    ...
    try {
      ...
      const startDateTime = `${newEvent.date}T${newEvent.startTime}:00Z`
      const endDateTime = newEvent.endTime
        ? `${newEvent.date}T${newEvent.endTime}:00Z`
        : `${newEvent.date}T${newEvent.startTime.split(':')[0]}:${parseInt(newEvent.startTime.split(':')[1]) + 30}:00Z`
