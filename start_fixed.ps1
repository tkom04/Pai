...
$pipExe = Join-Path $PWD "venv\Scripts\pip.exe"

$pythonVersion = & $pythonExe --version 2>&1
Write-Host "✓ Python found (venv): $pythonVersion" -ForegroundColor Green
...
$pipList = & $pipExe list 2>&1 | Out-String
...
$backendJob = Start-Job -ScriptBlock {
    ...
    & $PythonPath -m uvicorn app.main:app --host 0.0.0.0 --port 8080 --reload
} -ArgumentList $PWD, $pythonExe
...
if ((-not $skipFrontend) -and (Test-Path "frontend copy/package.json")) {
    ...
    $frontendJob = Start-Job -ScriptBlock {
        ...
        npm run dev
    } -ArgumentList (Join-Path $PWD "frontend copy")
